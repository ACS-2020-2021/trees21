...
public class BinarySearchTree<T extends Comparable<T>> {
  ...
  private TreeNode<T> getSmallest(TreeNode<T> tree) {// 9
    return tree;
  }
  ...
  private boolean search(T val, TreeNode<T> tree) {// 10...done!
    if (tree != null) {
  ...
  private TreeNode remove(T val, TreeNode<T> tree) {// 11...done!
    if (tree == null) {
      return null;
    }
    T treeValue = tree.getValue();
  ...
  private String toString(TreeNode tree) {// 12...done!
    if (tree == null)
      return "";
